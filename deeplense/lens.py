...
# Lenstronomy imports become very slow in this environment when numba JIT is enabled.
...
class DeepLens(object):
    ...
    def __init__(self,axion_mass=None,H0=70,Om0=0.3,Ob0=0.05,z_halo=0.5,z_gal=1.0):
        ...
        self.cosmo = FlatLambdaCDM(H0=self.H0, Om0=self.Om0, Ob0=self.Ob0)
        self.astropy_instance = self.cosmo
    ...
    def draw_old_cdm_sub_masses(self,m_sub_min=1e6,m_sub_max=1e10,n_sub=25,beta=-0.9):
        ...
        u = np.random.uniform(0, 1, size=n_halos)
    ...
    def mass_to_radius(self,Mass,redshift_halo,redshift_gal):
        ...
        DL = self.cosmo.luminosity_distance(redshift_halo).to(u.m)

        # Luminosity distance to lensed galaxy
        DS = self.cosmo.luminosity_distance(redshift_gal).to(u.m)
    ...
    def make_old_cdm(self, n_sub=25, m_sub_min=1e6, m_sub_max=1e10, beta=-0.9):
        """
    ...
    def simple_sim(self, num_pix=150, delta_pix=0.05, background_rms=1.0e-2, exp_time=None, fwhm=0.087):
        ...
        # data specifics
        exp_time = exp_time or 10**np.random.uniform(3,3.5)  # arbitrary units
        numPix = int(num_pix)
        deltaPix = float(delta_pix)
        fwhm = float(fwhm)
    ...
    def simple_sim_2(self, numpix=64):
        """
        ...
        if not hasattr(self, 'kwargs_single_band'):
            raise ValueError('Instrument must be configured before calling simple_sim_2().')
        ...
        kwargs_model_physical = {'lens_model_list': self.lens_model_list,  # list of lens models to be used
                              ...
                              'z_source_convention': self.z_gal,
                              'z_source': self.z_gal,} 

        
        #######################################################################
        numpix = int(numpix)  # number of pixels per axis of the image to be modelled
    ...
    def set_instrument(self,inst_name):
        ...
        # Note .lower() here is just to make string lower case
            ...
            raise ValueError(f'Unsupported instrument: {inst_name}')
